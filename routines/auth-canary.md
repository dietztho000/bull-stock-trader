...
If that command exits non-zero (401, 403, network error, etc.):
  bash scripts/run-log.sh end auth-canary fail
  bash scripts/discord.sh --type=error "auth preflight failed in auth-canary — check ALPACA_API_KEY / ALPACA_SECRET_KEY / ALPACA_ENDPOINT on the routine"
