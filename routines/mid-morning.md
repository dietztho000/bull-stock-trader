...
the bot list from `bash scripts/bots.sh list --routine=mid-morning`
...
```bash
: "${FLEET_DIGEST:=$(mktemp -t fleet-digest-mid-morning.XXXXXX)}"
export FLEET_DIGEST
```
...
STEP 6 — ALWAYS stash this bot's mid-morning digest block into the fleet
...
STEP 7 — Send ONE consolidated mid-morning scan to the midday channel
...
empty (every bot skipped preflight), send the one-liner "🎯 Mid-morning
