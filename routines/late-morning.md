...
the bot list from `bash scripts/bots.sh list --routine=late-morning`
...
```bash
: "${FLEET_DIGEST:=$(mktemp -t fleet-digest-late-morning.XXXXXX)}"
export FLEET_DIGEST
```
...
STEP 6 — ALWAYS stash this bot's late-morning digest block into the fleet
...
STEP 7 — Send ONE consolidated late-morning scan to the midday channel
...
empty (every bot skipped preflight), send the one-liner "🎯 Late-morning
...
if git diff --cached --quiet; then
  ...
  git commit -m "late-morning $DATE ($(bash scripts/bots.sh count) bots)"
