...
const COPY: Record<
  AlpacaMode,
  { title: string; envKey: string; envSecret: string; danger: boolean }
> = {
  live: {
    title: "Live account",
...
export async function AccountIdentityTile({ mode }: { mode: AlpacaMode }) {
      ...
      <Card title={c.title}>
        <div className="space-y-2 py-2">
          ...
          <div className="text-xs text-[var(--color-muted)] leading-relaxed">
            ...
            means the keys don&apos;t match the endpoint. Verify {c.envKey} /{" "}
            {c.envSecret} in .env.
          </div>
